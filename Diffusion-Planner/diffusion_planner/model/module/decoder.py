...
class RouteEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # x的shape是(B, route_num, lane_len, 4)，其中4表示(x, y, x->x', y->y')
        x = x[..., :4]

        B, P, V, _ = x.shape
        # mask_v的shape是(B, P, V)，表示每个点是否有效
        mask_v = torch.sum(torch.ne(x[..., :4], 0), dim=-1).to(x.device) == 0
        # mask_p的shape是(B, P)，表示每个路径是否有效
        mask_p = torch.sum(~mask_v, dim=-1) == 0
        # mask_b的shape是(B,)，表示每个batch是否有效
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, cross_c, route_lanes, neighbor_current_mask):
        ...
        # 将route_encoding和t结合作为condition输入到每个DiT block中
        y = route_encoding
