...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask):
        #Q-only 归一化更强调稳定 logits，保留 K/V 的原始尺度
...
class AgentFusionEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        neighbor_type = x[:, :, -1, 8:]
        # 对x的最后一维度只取出来(x, y, cos, sin, vx, vy, w, l)
        x = x[..., :8]
        ...
        pos[..., -3:] = 0.0
        pos[..., -3] = 1.0
        # 经过处理之后pos的size变成(B, P, 7)且最后三维是[1,0,0]，[x, y, cos, sin, type_one_hot(3)]
        
        B, P, V, _ = x.shape
        # mask_v: (B, P, V) 代表每个时间步的点是否为有效点，只有8个维度全为0才是无效点
        mask_v = torch.sum(torch.ne(x[..., :8], 0), dim=-1).to(x.device) == 0
        # mask_p: (B, P) 代表每个实体是否为有效实体，只有该实体所有时间步都是无效点才是无效实体
        mask_p = torch.sum(~mask_v, dim=-1) == 0
        # 给每个时间步加上一个是否为有效点的标志位
        # x的size变成(B, P, V, 8+1)，~mask_v是为了将=0的点变成1，表示无效点
        ...
        # pooling 对时间维度做平均池化，得到每个实体的固定长度表示
        # x的size变成(B*P_valid, channels)
        ...
        #返回(B, P, hidden_dim), (B, P), (B, P, 7)分别代表编码结果，有效实体mask，实体位置
...
class StaticFusionEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        pos[..., -3:] = 0.0
        pos[..., -2] = 1.0
        # 经过处理之后pos的size变成(B, P, 7)且最后三维是[0,1,0]，[x, y, cos, sin, type_one_hot(3)]

        x_result = torch.zeros((B * P, self._hidden_dim), device=x.device)

        # 代表每个实体是否为有效实体，只有该实体所有维度都是无效点才是无效实体
        mask_p = torch.sum(torch.ne(x[..., :10], 0), dim=-1).to(x.device) == 0

        valid_indices = ~mask_p.view(-1) 

        # valid_indices代表有效实体的索引
        ...
        # 返回的(B, P, hidden_dim), (B, P), (B, P, 7)分别代表编码结果，有效实体mask，实体位置
...
class LaneFusionEncoder(nn.Module):
    ...
    def forward(self, x, speed_limit, has_speed_limit):
        ...
        # heading = torch.atan2(delta_y, delta_x)
        ...
        if (~has_speed_limit).sum() > 0:
            # speed_limit_no_limit的size是(N_no_limit, channels)
